...
export default function GeoRiskPanel({ geo }) {
  ...
  const srcLabel = geo?.source === 'gdelt' ? 'GDELT 24h' : geo?.source === 'rss-fallback' ? 'RSS 24h' : '24h';
  return (
    <div className="hud-panel p-3.5 h-full">
      <div className="flex items-center justify-between pb-2 mb-2 border-b border-white/5">
        ...
        <span className={`text-[10px] font-mono ${geo?.score >= 50 ? 'text-amber-400' : 'text-slate-400'}`}>
          {geo?.live ? `${geo.score} / ${geo.level} · ${srcLabel}` : '—'}
        </span>
      </div>
      <p className="text-[11px] font-mono text-slate-400 mb-2">{geo?.goldImplication || '24h gold-relevant geopolitical document volume.'}</p>
      <div className="space-y-1.5 max-h-[220px] overflow-y-auto custom-scrollbar">
        {headlines.length === 0 ? (
          <p className="text-xs font-mono text-slate-500">No geopolitical articles loaded yet.</p>
        ) : headlines.map((h, i) => (
