...
export default function MacroDriversGrid({ marketData }) {
  ...
  const yieldSpread = marketData?.yieldCurveSpread;
  ...
  const cards = [
    ...
    },
    {
      ...
      data: assets.US10Y,
      extra: `Real yield: ${realYield ?? '—'}% (TNX − T10YIE)`,
      suffix: '%',
      inverse: true
    },
    {
    ...
    },
    {
      ...
      data: assets.US02Y,
      extra: `10Y-2Y: ${yieldSpread == null ? '—' : `${yieldSpread > 0 ? '+' : ''}${yieldSpread}%`}`,
      suffix: '%',
      inverse: true
    },
    {
    ...
    },
    {
      id: 'USDJPY',
      title: 'USD/JPY',
      role: 'Yen carry / USD liquidity proxy',
      data: assets.USDJPY,
      extra: 'Stronger USDJPY often tracks a firmer dollar',
      inverse: true
    },
    {
      id: 'SPX',
      title: 'S&P 500',
      role: 'Risk appetite vs safe-haven bid',
      data: assets.SPX,
      extra: 'Equity stress can lift gold independently of DXY',
      inverse: false
    },
    {
      id: 'TLT',
      title: 'TLT (Long Treasuries)',
      role: 'Duration bid / falling yields proxy',
      data: assets.TLT,
      extra: 'Rising TLT = falling long yields, gold-friendly',
      inverse: false
    },
    {
      id: 'VIX',
    ...
    <div className="hud-panel p-4">
      ...
      <div className="flex items-center justify-between pb-3 mb-3 border-b border-white/5">
        ...
        <div className="text-[11px] font-mono text-slate-400">
          Rolling Correlation Window: <strong className="text-slate-200">15-Minute Time-Aligned Returns</strong>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {cards.map((card) => {
          const item = card.data || { price: null, change: 0, changePercent: 0, correlation: null, momentum: 'UNAVAILABLE' };
          ...
          const priceLabel = item.price == null
            ...
            : card.suffix
              ...
              : Number(item.price).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 });
            ...
            >
              ...
              <div className="mt-3 pt-2 border-t border-white/5">
                <div className="flex items-center justify-between text-[10px] font-mono text-slate-400 mb-1">
                  <span>Rolling Correlation (15m)</span>
                  <span className={`font-bold ${corr == null ? 'text-slate-500' : corr > 0.3 ? 'text-emerald-400' : corr < -0.3 ? 'text-cyan-400' : 'text-slate-300'}`}>
                ...
                <div className="w-full h-1.5 bg-slate-900 rounded-full overflow-hidden relative border border-white/5">
                  ...
                  {corr != null && (
                    <div
                      ...
                      style={{
                        width: `${Math.min(50, Math.abs(corr) * 50)}%`,
                        marginLeft: corr >= 0 ? '50%' : `${50 - Math.min(50, Math.abs(corr) * 50)}%`
                      }}
                    />
                  )}
                </div>
